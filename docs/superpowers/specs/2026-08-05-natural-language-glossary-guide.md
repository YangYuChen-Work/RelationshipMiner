...
项目首版可使用 [项目词汇表测试夹具](examples/natural-language-glossary-project-fixture.yaml)。它保留了当前提供的需求、工艺、装配与项目映射；加载时仍会对表名和既有 `name` / `class_name` 契约做校验。短词或宽泛别名（例如 `样`）只产生候选证据，不会绕过模型的语义判断或后端校验。
...
4. YAML 未命中、证据不足或词条歧义时，模型仍须从完整核心表目录中，结合用户描述、表业务摘要和字段元数据做语义选表；这保证词汇表只是可维护的增强，而不是遗漏表的单点故障。只有模型也无法可靠选择时，才返回 `needs_clarification`。
