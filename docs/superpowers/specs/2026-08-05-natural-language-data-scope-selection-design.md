...
当前仅维护 10 张核心表，模型必须看到全部目录的表级摘要；词汇表只影响排序、命中理由和模型证据，绝不作为硬过滤条件。YAML 没有命中、命中证据不足或词条存在歧义时，AI 必须继续依据用户完整描述、表业务摘要和字段元数据进行语义选表；只有 AI 也无法可靠判断时才返回 `needs_clarification`。表规模增长后再采用“命中表 + 语义 Top-K + 业务域核心表 + 明确点名表”的宽候选策略，并始终保留低召回兜底。
...
- `GlossaryRepository`：加载首版 `aliases -> tables` YAML、校验结构并给出 `glossaryVersion`。
- `CandidateRanker`：归一化输入，产生别名字面量和元数据匹配证据；不做最终选择。
...
`needs_clarification` 返回 `reason_code`、`guidance` 和至多两个 `suggested_questions`；`unavailable` 返回 HTTP 503、`reason_code` 与安全提示。模型自报的 0–1 `confidence` 不作为接受门槛，也不必在第一版返回。接受 `selected` 的条件是结构合法、所有表字段均在快照内、表数不超过 10、模型未报告关键歧义，并且每个选表具有可解释的词汇或元数据证据。
...
第一版格式只包含 `mappings`：每个映射项有一个或多个 `aliases`，并指向一个或多个 `tables`。例如“客户”可只指向 `customer`，而“订单”在同一个映射项中同时指向销售和采购订单。命中后会为每张表产生歧义证据，而不是配置错误或硬过滤；模型结合完整描述、全部核心表目录与字段元数据完成最终选择。

同一别名应集中在一个映射项中；若该别名可指多个表，将这些表一起列在 `tables`，而不是复制多个映射项。空值、映射项内重复值和不存在或不可分析的表属于 `GLOSSARY_INVALID`。首版不包含 YAML 正则、权重、字段词条、排除词或前端编辑，优先以规范、可审查的字面量映射满足 10 张核心表的维护需求。
...
后端单元和接口测试覆盖：目录快照与不合格表过滤、词汇 YAML/别名字面量/多表歧义/重复别名、候选证据、模型请求、未指定字段时展开所有合法辅助字段、明确字段时只保留指定字段、未知表字段、隐式字段误选、表数上限、模型超时、非法 JSON、元数据不可用、HTTP 状态以及不泄露记录或内部信息。
...
以下为硬性验收条件：不存在表、无权限表（鉴权接入后）、非法字段、超过 10 表以及失败时污染 `selectedTables` 的发生率均为 0。核心语料的完整表集合准确率目标为至少 90%，并在词汇表、模型或选择器版本变化时回归执行。
