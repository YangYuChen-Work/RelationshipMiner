...
function AnalysisNotice() {
  ...
  if (analysisStatus === "partial") {
    ...
    return <section role="status" className="mx-3 mt-3 rounded-lg border border-amber-400/30 bg-amber-400/10 px-4 py-3 text-sm text-amber-100"><p className="font-semibold">分析未完成，正在显示可用关系。</p><p className="mt-1 text-xs">候选关系：已完成 {completed} · 待处理 {pending} · 失败 {failed}</p>{warnings.length > 0 && <ul className="mt-2 list-disc pl-5 text-xs">{warnings.map((warning, index) => <li key={`${warning}-${index}`}>{warning}</li>)}</ul>}</section>;
  }
